    ...
    <div className="h-[89vh] flex flex-col max-w-[1400px] m-auto">
      ...
      <div>
        <h1 className="bg-blue-600 font-semibold text-white flex p-5 gap-2 justify-center rounded-t-md py-7">
          <h2>{numnight}</h2> night in {fromdate} In cabin {cabin_id} - {todate}
        </h1>
      </div>
      <div className="bg-white p-7 flex flex-col gap-5 h-[400px] leading-10">
        {guestdata.map((el) => (
        ...
        <div
          className={clsx(` 
              ${status === 'Unconfirmed' && 'font-semibold text-amber-600'} ${status === 'Checked In' && 'bg-green-100 text-green-600'} flex justify-between p-5 bg-amber-200 rounded-md font-semibold`)}
        >
          <h1
            className={clsx(
